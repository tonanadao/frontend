...
const __1 = require("..");
const client = new ton_1.TonClient({ endpoint: 'https://toncenter.com/api/v2/jsonRPC' });
const address = ton_1.Address.parse('Ef-P_TOdwcCh0AXHhBpICDMxStxHenWdLCDLNH5QcNpwMMn2');
describe('PowGiverContract', () => {
    it('should get resulsts', async () => {
        let contract = await __1.PowGiverContract.create(address, client);
        let powParams = await contract.getPowParams();
        let contractState = await client.getContractState(address);
        let stateBoc = ton_1.Cell.fromBoc(contractState.data)[0];
        let powParams2 = await __1.PowGiverContract.extractPowParamsFromState(stateBoc);
    ...
    it('should create mining message', async () => {
        ...
        let msg = __1.PowGiverContract.createMiningMessage({
            giver: ton_1.Address.parse('Ef91o4NNTryJ-Cw3sDGt9OTiafmETdVFUMvylQdFPoOxInls'),
            wallet: ton_1.Address.parse('EQCD39VS5jcptHL8vMjEXrzGaRcCVYto7HUn4bpAOg8xqB2N'),
            seed: goldenSeed,
            random: goldenRandom,
    ...
    it('should check mining result', async () => {
        ...
        const job = __1.PowGiverContract.createMiningJob({
            ...
            wallet: ton_1.Address.parse('EQDsa6i81-_ZJ63CGCJGY_veennFLdmMUkrfnkDr716vdKQe'),
            expiresSec: 1636976680
        });
        expect(job.equals(Buffer.from(jobGolden, 'hex'))).toBe(true);
        // Job hash
        let res = await __1.PowGiverContract.checkMiningJobHash({
            seed: Buffer.from('3da0cb38cc79a0e71ced1fa24507ee46', 'hex'),
            random: Buffer.from('65ab1bde2ba1fd575f2b4e46cda7f94e32f2581438655f68a51ce3da97660638', 'hex'),
            wallet: ton_1.Address.parse('EQDsa6i81-_ZJ63CGCJGY_veennFLdmMUkrfnkDr716vdKQe'),
            expiresSec: 1636976680,
    ...
    it('should parse pow giver message', async () => {
        const cell = ton_1.Cell.fromBoc(Buffer.from('te6cckEBAQEAewAA8k1pbmUAYaOB/QAWGNfMq5Dk3Rz3qex1lBYBNwmb3T0thJoIC9YXFUk/QlrMM6dKTDdd8OnD8NObEAAAAAAAV2uEhAAAAA8AAACDSJ2epCU+cIujTv7nzc1IQlrMM6dKTDdd8OnD8NObEAAAAAAAV2uEhAAAAA8AAABj6jgW', 'base64'))[0];
        const parsed = __1.PowGiverContract.parseMiningMessage(cell.beginParse());
    ...
    it('should parse pow giver message from masterchain', async () => {
        const cell = ton_1.Cell.fromBoc(Buffer.from('te6cckEBAQEAewAA8k1pbmX8Xz4yT0h3NIIqk9FQKQWkLS2FYzDXjMm+hValSGMGlDb8Bxs7fxx5vJl6bIuDwi6f3QdfKtv1QalzCb5LrNbh3TiScIUmeGg5wci/1mQDip86Y2twfxx5vJl6bIuDwi6f3QdfKtv1QalzCb5LrNbh3TiScIXpl/qP', 'base64'))[0];
        const parsed = __1.PowGiverContract.parseMiningMessage(cell.beginParse());
        expect(parsed).not.toBeNull();
        expect(parsed).not.toBeUndefined();
        expect(parsed.op).toBe('mine');
